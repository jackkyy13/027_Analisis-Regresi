...
data <- read_excel("C:\\Users\\ACER\\Downloads\\NSC-2025.xlsx", sheet="Sheet2")
...
Saat ini terdapat 1 peubah Y dan 9 peubah X, misal ingin dilakukan eksplorasi dan analisis terkait peubah X2

```{r}
plot(data$X3, data$Y,
     main = "Presentase Penduduk Miskin vs Indeks Pembangunan Manusia",
     xlab = "PPM",
     ylab = "IPM")
```
...
cat("Koefisien b1:", b1, "\n")
```
#DAH CUKUP SAMPE INI AJA KALO TUJUANNYA UNTUK PREDIKSI DOANG
...
$$\hat{Y} = 84.25520 - 0.93772 X_3$$

dengan Y adalah Indeks Pembangunan Manusia dan X3 adalah persentase penduduk miskin.

Didapatkan nilai $\hat\beta_0 = 84.25520$ artinya nilai dugaan rataan indeks pembangunan manusia ketika presentase penduduk miskin bernilai 0 persen adalah 84.25520%, akan tetapi nilai 0 tidak ada dalam selang pengamatan presentase penduduk miskin sehingga $\hat\beta_0$ tidak dapat didefinisikan lebih lanjut

Nilai $\hat\beta_0 = -0.93772$ artinya nilai dugaan perubahan dugaan rataan Indeks Pembangunan Manusia jika presentase penduduk miskin berubah satu tahun adalah sebesar -0,93772%

Koefisien determinasi yang dihasilkan adalah 0.4831 atau 48,31% artinya indeks pembangunan manusia yang dapat dijelaskan oleh persentase penduduk miskin adalah sebesar 48,31%, sisanya yaitu 51,69% dijelaskan oleh peubah lain yang tidak dimasukkan dalam model.
