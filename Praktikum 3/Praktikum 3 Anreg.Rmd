...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\ACER\\OneDrive\\Dokumen\\Produksi_Beras_2023_Analisis_Regresi.xlsx", sheet="Sheet1")
dt
```

```{r}
y <- dt$"Produksi Beras (Ton)"
x <- dt$"Produksi Padi (Ton)"
n <- length(dt)
```
...
Berdasarkan perhitungan, didapat dugaan persamaan regresi linear sebagai berikut. Dugaan persamaan tersebut dapat pula disebut sebagai nilai harapan dari peubah respons Y (Produksi Beras (Ton))

$$E[\hat{Y}] = \hat{Y} = -278.5 + 0.5763 X$$
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{2.831974} \approx 41184483$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar -278.5 dan 0.5763. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [-6693.829 ,  6136.908] dan untuk parameter $β_1$ adalah [0.5752857 ,  0.5774114]
...
```{r}
amatan.diduga <- data.frame(x=275941)
predict(model, amatan.diduga, interval = "confidence")
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=275941 adalah 158759.7. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=275941 berada dalam selang 156440.5 hingga 161079.
...
Misalkan kita ingin menduga nilai individu amatan ketika nilai x=275941. Kita dapat memanfaatkan fungsi `predict` untuk mendapatkan nilai dugaan dan selang kepercayaannya sebagai berikut.

```{r}
predict(model, amatan.diduga, interval = "prediction")
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=275941 adalah 158759.7. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=275941 berada dalam selang 145539.4 hingga 171980.1.